...
# mln = number of line in middle in between 2 markers
mln = 100 #50 #75,100,125
...
def connect_middle_100(image, output_image_path, coordinates):
    ...
    # Calculate the increments for dividing the sides into mln pieces
    left_y_increment = (left_bottom[0][1] - left_top[0][1]) / mln
    right_y_increment = (right_bottom[0][1] - right_top[0][1]) / mln
...
def process_and_save_csv_files(file_pattern='sample_name_*.csv', input_dir='intensity_of_samples'):
    """
    Processes each CSV file in the given subfolder matching the pattern, sorts, transposes, and saves them.
    """
    # Ensure input_dir is a Path
    input_dir = Path(input_dir)

    # Build the glob pattern (folder + file pattern)
...
def merge_and_sort_files(processed_dfs):
    ...
    sorted_df.to_csv("ready_for_normalization.csv", index=False)
    return sorted_df


def quantile_normalize_rowwise(final_sorted_df, tie_method: str = "average"):
    """
    Quantile normalization row-wise with proper tie handling 
    """
    df = final_sorted_df.copy()
    ...
    # 1) Sort values within each row
    sorted_vals = np.sort(X, axis=1)
    
    # 2) Reference distribution
    mean_vector = sorted_vals.mean(axis=0)
    
    # 3) Calculate ranks with tie handling
    ranks = df_num.rank(axis=1, method=tie_method, ascending=True).to_numpy()
    
    # 4) Interpolate onto reference distribution
    positions = np.arange(1, df_num.shape[1] + 1, dtype=float)
    normalized_vals = np.interp(ranks, positions, mean_vector)
    
    # Rebuild DataFrame
    df_norm_num = pd.DataFrame(normalized_vals, 
                              index=df_num.index, 
                              columns=df_num.columns).round(2)
    
    # Reattach non-numeric columns 
    if non_num_cols:
        df_norm = pd.concat(
            [df[non_num_cols].reset_index(drop=True),
             df_norm_num.reset_index(drop=True)],
            axis=1
        )
    ...
    df_norm.to_csv("normalized_df.csv", index=False)
    return df_norm

def ready_for_ml(combined_df,classification_file):
    df= combined_df

    # Get the directory where the script/notebook is running
    output_dir = Path.cwd()
    ...
    val_df.to_csv(output_dir / 'normalized_validation.csv', index=False)
    non_val_df.to_csv(output_dir / 'normalized_samples.csv', index=False)
    ...
    df2_sorted.to_csv(output_dir / 'ready_for_ml_class_1.csv', index=False)
